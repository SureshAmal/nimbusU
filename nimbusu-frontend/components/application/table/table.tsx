...
import type { ComponentPropsWithRef, HTMLAttributes, ReactNode, Ref, TdHTMLAttributes, ThHTMLAttributes, ReactElement } from "react";
import { createContext, isValidElement, useContext, forwardRef } from "react";
...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("overflow-hidden rounded-xls shadow-xs ring-1 ring-secondary", className)}>
                {children}
            </div>
    ...
    return (
        <TableContext.Provider value={{ size: context?.size ?? size }}>
            <div className="w-full">
                <AriaTable className={(state) => cx("w-full border-collapse", typeof className === "function" ? className(state) : className)} {...props} />
            </div>
        </TableContext.Provider>
    );
};
        ...
        <AriaTableHeader
            ...
            className={(state) =>
                cx(
                    ...
                    // Row border—using standard bottom border rather than absolute pseudo-element to avoid z-index/alignment issues with sticky headers
                    bordered && "border-b border-secondary",

                    typeof className === "function" ? className(state) : className,
                    // apply bg-secondary if no specific bg color is provided by className override
                    !(typeof className === "string" && className.includes("bg-")) && "bg-secondary"
                )
            }
        >
...
const TableHead = ({ className, tooltip, label, children, ...props }: TableHeadProps) => {
    ...
    return (
        <AriaColumn
            ...
            className={(state) =>
                cx(
                    "relative p-0 px-4 py-1.5 outline-hidden focus-visible:z-1 focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-offset-bg-primary focus-visible:ring-inset",
...
const TableRowInner = forwardRef(<T extends object>({ columns, children, className, highlightSelectedRow = true, onContextMenu, ...props }: TableRowProps<T> & { onContextMenu?: (e: React.MouseEvent) => void }, ref: Ref<HTMLTableRowElement>) => {
    const { size } = useContext(TableContext);
    const { selectionBehavior } = useTableOptions();
    ...
    return (
        <AriaRow
            ...
            className={(state) =>
                cx(
                    "relative outline-focus-ring transition-colors after:pointer-events-none hover:bg-secondary focus-visible:outline-2 focus-visible:-outline-offset-2 cursor-context-menu",
                    size === "sm" ? "h-9" : "h-11",
...
const TableRow = TableRowInner as <T extends object>(props: TableRowProps<T> & { ref?: Ref<HTMLTableRowElement> }) => ReactElement;
